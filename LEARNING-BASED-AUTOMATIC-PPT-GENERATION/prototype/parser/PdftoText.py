...
class PdfToText:
	...
	print(bytes_as_hex)
        if bytes_as_hex.startswith('ffd8'):
            file_type = '.jpeg'
        elif bytes_as_hex.startswith('4889'):
            file_type = '.png'
    ...
    def parse_lt_objs(self, lt_objs, page_number, images_folder, text=[],f=0):
        ...
        for lt_obj in lt_objs:
		    ...
		    print ptoh_file," f value", f
		    if("tutorial" in ptoh_file and f == 1):
		    	inp_file_name = ptoh_file.split('/')[len(ptoh_file.split('/'))-1]
		    	#os.system("pdftohtml "+ptoh_file+" -f " + str(page_number) + " -l " + str(page_number))
		    	#print("pdftohtml "+ptoh_file+" -f " + str(page_number) + " -l " + str(page_number)+" "+inp_file_name)
                    	#text_content.append('figure '+ str(page_number)+":"+str(f)+" "+inp_file_name.split('.')[0].replace('-','_'))
		    	print 'figure '+ str(page_number)+"_"+str(f)+" "+inp_file_name+ " not added"
	            else:
		    	inp_file_name = ptoh_file.split('/')[len(ptoh_file.split('/'))-1]
		    	os.system("pdftohtml "+ptoh_file+" -f " + str(page_number) + " -l " + str(page_number))
		    	print("pdftohtml "+ptoh_file+" -f " + str(page_number) + " -l " + str(page_number)+" "+inp_file_name)
                    	text_content.append('figure '+ str(page_number)+"_"+str(f)+" "+inp_file_name.split('.')[0].replace('-','_'))
		    	print 'figure '+ str(page_number)+"_"+str(f)+" "+inp_file_name
		...
		f=f+1
                ...
                text_content.append(self.parse_lt_objs(lt_obj, page_number, images_folder, text_content,f))
